...
function M.executable_or_error(name, binary, ...)
    ...
    if not path.exists(p) then
        if M.installed(name) then
            log.fatal(string.format("package '%s' installed, but program '%s' not found", name, binary))
        else
            log.fatal(string.format("program '%s' not found; try 'wf-pacman -Sy %s'", binary, name))
        end
    else
        return p
    end
